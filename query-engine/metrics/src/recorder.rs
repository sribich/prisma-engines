...
use metrics::{Counter, CounterFn, Gauge, GaugeFn, Histogram, HistogramFn, Key, Recorder, SharedString, Unit};
use metrics::{KeyName, Metadata};
use tracing::trace;
...
impl Recorder for MetricRecorder {
    fn describe_counter(&self, key_name: KeyName, _unit: Option<Unit>, description: SharedString) {
        self.register_description(key_name.as_str(), description.as_ref());
    }
    ...
    fn register_counter(&self, key: &Key, metadata: &Metadata) -> Counter {
        Counter::from_arc(Arc::new(MetricHandle(key.clone())))
    }

    fn register_gauge(&self, key: &Key, metadata: &Metadata) -> Gauge {
        Gauge::from_arc(Arc::new(MetricHandle(key.clone())))
    }

    fn register_histogram(&self, key: &Key, metadata: &Metadata) -> Histogram {
